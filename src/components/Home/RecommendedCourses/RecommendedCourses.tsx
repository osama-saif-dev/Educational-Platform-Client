...
import './RecommendedCoursesStyles.css';
...
export default function RecommendedCourses() {
    return (
        <motion.div variants={parentDiv} initial='hidden' whileInView='visible' viewport={{ once: true }} className='px-8 my-[50px]'>
            <motion.h1 variants={childYDiv} className='text-center sm:text-start text-[30px] sm:text-[40px] md:text-[50px] font-bold text-second'>Recommended Courses</motion.h1>
                    ...
                    {CourseImages && CourseImages.map((course) => (
                        <SwiperSlide className='mb-6 cursor-pointer border border-input' key={course.id}>
                            <div className="w-full relative h-full">
                                ...
                                <Image className="object-cover w-full" src={course.src} width={200} height={150} alt="Course Image" />
                            </div>
