...
import ResetPasswordLogo from '@/../public/ResetPassword.png';
import { motion } from 'motion/react';
import { childXDiv, childYDiv, parentDiv } from '@/services/ParentAndChildMotions';

export default function ResetPasswordImage() {
  return (
    <motion.div variants={parentDiv} initial='hidden' animate='visible' className='hidden lg:flex lg:w-[50%] relative h-[500px]'>
        ...
        <motion.div variants={childYDiv} className='absolute w-full left-[50%] top-[50%] transform -translate-x-1/2 -translate-y-1/2'>
          <Image className='w-full rounded-md' src={ResetPasswordLogo} width={200} alt='Main_Image'/>
        </motion.div>
    </motion.div>
  )
}
